...
def load_data():
    ...
    mod = pd.read_csv(config.TEST_MODIFIED_PATH) if config.TEST_MODIFIED_PATH.exists() else None
    return orig, mod
...
st.caption(
    f"Deployed model: **{meta['best_algorithm']}**  |  "
...
with tab_monitor:
    ...
    if mod_df is None:
        st.warning(
            "Run `python -m src.make_modified_test` first to generate the modified test set."
        )
    else:
        ...
        k1.metric(
            "RMSE", f"{mm['RMSE']:.2f}", f"{mm['RMSE'] - om['RMSE']:+.2f}", delta_color="inverse"
        )
        k2.metric("MAE", f"{mm['MAE']:.2f}", f"{mm['MAE'] - om['MAE']:+.2f}", delta_color="inverse")
        k3.metric("R²", f"{mm['R2']:.2f}", f"{mm['R2'] - om['R2']:+.2f}")
        k4.metric(
            "Mean predicted score",
            f"{mod_pred.mean():.1f}",
            f"{mod_pred.mean() - orig_pred.mean():+.1f}",
            delta_color="inverse",
        )
        ...
        st.metric(
            "Students flagged At-Risk", f"{r2:.0f}%", f"{r2 - r1:+.0f} pts", delta_color="inverse"
        )
        ...
        fig.add_vline(
            x=config.RISK_THRESHOLD, line_dash="dash", annotation_text="At-Risk threshold"
        )
        fig.update_layout(
            barmode="overlay",
        ...
        st.dataframe(
            drift_df.style.applymap(color, subset=["PSI"]),
            use_container_width=True,
            hide_index=True,
        )
        st.info(
            f"Dataset drift detected: **{report['summary']['dataset_drift']}** "
