...
def run_comparison():
    ...
    comparison = {
        "original": {**orig_m, "mean_prediction": float(orig_pred.mean()),
                     "at_risk_pct": float((orig_pred < config.RISK_THRESHOLD).mean() * 100)},
        "modified": {**mod_m, "mean_prediction": float(mod_pred.mean()),
                     "at_risk_pct": float((mod_pred < config.RISK_THRESHOLD).mean() * 100)},
    }
    comparison["delta"] = {
        "mean_prediction_drop": round(comparison["original"]["mean_prediction"]
                                      - comparison["modified"]["mean_prediction"], 2),
        "at_risk_pct_increase": round(comparison["modified"]["at_risk_pct"]
                                      - comparison["original"]["at_risk_pct"], 2),
        "rmse_increase": round(comparison["modified"]["RMSE"]
                               - comparison["original"]["RMSE"], 2),
    }
    ...
    ax.axvline(config.RISK_THRESHOLD, color="black", linestyle="--",
               label=f"At-Risk (<{config.RISK_THRESHOLD:.0f})")
    ax.set(title="Predicted Performance Score: Original vs Modified test set",
           xlabel="Predicted score", ylabel="Number of students")
    ax.legend()
    fig.tight_layout()
